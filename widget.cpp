    ...
    , ui(new Ui::Widget)
{
    ...
    for(int i = 0;i<fileList.count();i++)
       {
           ...
           musicList->addMedia(QUrl::fromLocalFile(aFile));
    ...
    //musicList->addMedia(QMediaContent(QUrl("qrc:/new/prefix1/music/Avid.mp3")));
    //musicList->addMedia(QMediaContent(QUrl("qrc:/new/prefix1/music/Running In The Dark.mp3")));
    //musicList->addMedia(QMediaContent(QUrl("qrc:/new/prefix1/music/Call of Silence.mp3")));
    //musicList->setPlaybackMode(QMediaPlaylist::Loop);
    QDir dir=QString(":/new/prefix1/music");
    player->setPlaylist(musicList);
    player->setVolume(0);  //音量
    ui->num_label->setText("歌曲数:"+QString::number(fileList.count()));
    if(fileList.count()>0)ui->love_list->setCurrentRow(0);
    //auto musiclist= dir.entryList(QStringList()<<"*.mp3"<<"*.wav");
    //qInfo()<<musiclist;
    //ui->love_list->addItems(musiclist);
...
void Widget::on_toolButton_2_released()
{
     ...
     int row = musicList->mediaCount();//获取歌曲数量
     int current  = musicList->currentIndex();//获取当前位置
     ui->love_list->item(current)->setForeground(QColor(186,186,186));
     if(--current<0){
     current=row;
     }
     else ui->love_list->item(current)->setForeground(QColor(221,65,65));
     musicList->setCurrentIndex(current);//设置当前音乐
     list_row=current;
     chose_jpg(list_row);
     if(Start_flag==0)player->play();
}
void Widget::on_toolButton_4_released()
{
     ...
     if(++current>row){
     current=0;
     ui->love_list->item(1)->setForeground(QColor(221,65,65));
     }else ui->love_list->item(current)->setForeground(QColor(221,65,65));
     musicList->setCurrentIndex(current);//设置当前音乐
     list_row=current;
     chose_jpg(list_row);
